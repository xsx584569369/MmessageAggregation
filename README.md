...
设置里可：切换深/浅色主题、开关数据源、增删关键词（命中即高亮/强提醒）、桌面通知开关、仅关键词提醒、自动刷新间隔、**钉钉推送**。左下角「＋ 管理监控公司」可在界面里增删公司（含 secid/关键词/英文名），无需改代码。

### 代理（内网 / 无外网环境）

设置 → 「代理」：填 `http://主机:端口` 或 `socks5://主机:端口`（留空=直连）。配置后所有请求（含 Google/Yahoo 海外源、检查更新）都经代理。
- 国内无外网时，Google/Yahoo 会超时（`rc=28`）；要么填代理，要么在「数据源」里关掉这两个海外源（东财仍提供海外公司的中文资讯）。

### 钉钉推送

设置 → 「钉钉推送」：填群里「自定义机器人」的 Webhook，有新资讯时自动推到钉钉群（程序需在后台/托盘运行）。
- 机器人安全设置选「加签」→ 把 `SEC…` 密钥填到「加签密钥」
- 或选「自定义关键词」→ 把关键词填到「关键词」框
- 「发送测试消息」可即时验证配置是否正确
- 是否"仅命中关键词才推"跟随「仅命中关键词才提醒」开关
